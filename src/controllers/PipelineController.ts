  ...
  public parseArguments(args: string[]): IPipelineArgs {
    ...
    for (const arg of args) {
      if (arg.startsWith('--template=')) {
        ...
        parsed.action = arg.split('=')[1] as 'deploy' | 'undeploy' | 'validate' | 'status';
      }
    }
    ...
    return {
      ...
      action: parsed.action || (process.env.ACTION as 'deploy' | 'undeploy' | 'validate') || 'deploy'
    };
  }
...
Usage:
  pipeline --template=<template-name> --config=<config-file> --action=<action> --stack=<id>

Options:
  --template=<name>    Template name to use (required)
  --stack=<id>         Stack identifier (optional) (default: autogenerated, e.g., 'K2025071525')
...
Examples:
  pipeline --template=atlas.basic --config=cfg/config.json --action=deploy --stack=K2025071525
  pipeline --template=ops.manager --config=production.json --action=undeploy --stack=K2025071525
