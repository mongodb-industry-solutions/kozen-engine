...
import { pulumiProgram } from "../tools/program";
    ...
    public async deploy(config: IStackOptions): Promise<IResult> {
        try {
            const stack = await LocalWorkspace.createOrSelectStack(
                {
                    stackName: "stg",
                    projectName: "tony-project-55",
                    program: async () => {
                        // Define el proveedor de Kubernetes con el kubeconfig
                        const kubeconfigPath = 'C:\\Users\\tonyk\\.kube\\config'; // `${process.env.HOME}/.kube/config`;
            ...
            await stack.setAllConfig({
                ...
                "aws:accessKey": {
                    "value": "AKIAUALWIMQRFNDRB2NX",
                    "secret": true
                },
                "aws:secretKey": {
                    "value": "w0XOlxR1L7uUkykhSN1fyrYY2RLJj3s7evhKkPDp",
                    "secret": true
                }
            });
