  ...
  private castToAtlasConfig(input?: IStruct): AtlasConfig {
    ...
    return {
      ...
      providerInstanceSizeName:
        (input.providerInstanceSizeName as string) || "M10",
      cloudBackup:
        input.cloudBackup !== undefined ? Boolean(input.cloudBackup) : true,
...
export class KubernetesController extends KzComponent {
  ...
  private castToKubernetesConfig(input?: IStruct): KubernetesConfig {
    return {
      ...
      serviceType:
        (input?.serviceType as "ClusterIP" | "NodePort" | "LoadBalancer") ||
        "ClusterIP",
